...
import new
import dis
#import email   # can't import this for Mac Build
...
class CloudPickler(pickle.Pickler):
    ...
    #python2.6+ supports slice pickling. some py2.5 extensions might
    #as well.  We just test it
    try:
    ...
    def save_codeobject(self, obj, pack=struct.pack):
        ...
        args = (
            ...
            obj.co_firstlineno, obj.co_lnotab,
    ...
    def save_function(self, obj, name=None, pack=struct.pack):
        ...
        if not self.savedDjangoEnv:
            #hack for django - if we detect the settings module, we
            #transport it
            django_settings = os.environ.get('DJANGO_SETTINGS_MODULE', '')
            if django_settings:
                django_mod = sys.modules.get(django_settings)
                if django_mod:
                    cloudLog.debug('Transporting django settings %s',
                                   django_mod)
                    self.savedDjangoEnv = True
                    self.modules.add(django_mod)
                    self.save_reduce(django_settings_load,
                                     (django_mod.__name__,), obj=django_mod)
    ...
    def extract_func_data(func):
        ...
        for var in func_global_refs:
            #Some names, such as class functions are not global - we
            #don't need them
        ...
        def get_contents(cell):
            try:
                ...
                raise pickle.PicklingError(
                    'Function to be pickled has free variables that are '
    ...
    def save_instancemethod(self, obj):
        ...
        self.save_reduce(types.MethodType, (obj.im_func, obj.im_self,
    ...
    def save_reduce(self, func, args, state=None,
        ...
        Change only affects protocol level 2 (which is always used by
        PiCloud"""
    ...
    #python2.6+ supports xrange pickling. some py2.5 extensions might
    #as well.  We just test it
    try:
        xrange(0).__reduce__()
            ...
            for tst_mod_name in self.numpy_tst_mods:
                ...
                if tst_mod:
                    if name in tst_mod.__dict__:
                        ...
                        return
            raise pickle.PicklingError(
                'cannot save %s. Cannot resolve what module it '
                'is defined in' % str(obj))
        ...
        if isinstance(modname, str):
            try:
                ...
                sys.stderr.write(
                    ...
                    'Your function may unexpectedly error due to this '
                    'import failing; A version mismatch is likely. '
                    'Specific error was %s\n\n' % (modname, str(i)))
            else:
